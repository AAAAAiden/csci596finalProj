...
int main(int argc, char **argv) {
  ...
  // Initialize parameters, topology, configuration, and accelerations
  init_params();
  set_topology();
  init_conf();
  atom_copy();
  ...
  cpu1 = MPI_Wtime(); // Start timing

  // Parallel MD loop
  #pragma omp parallel default(none) shared(StepLimit, StepAvg, cpu1, sid, comt, cpu) private(stepCount)
  {
    #pragma omp for schedule(dynamic) nowait
    for (stepCount = 1; stepCount <= StepLimit; stepCount++) {
      #pragma omp critical
      {
      single_step(); // Perform one MD step
      }

      if (stepCount % StepAvg == 0) {
        #pragma omp critical
        {
          eval_props();  // Evaluate properties (kinetic, potential, total energy)
        }
      }
    }
    ...
    #pragma omp master
    {
      cpu = MPI_Wtime() - cpu1; // Record elapsed time
      if (sid == 0) {
...
----------------------------------------------------------------------*/
  ...
  double rrCut,lpe; // Note: added

  double lpe_td[nthrd]; // Note: added
...
// Note: added
  ...
  for (mc[0]=mofst[0]+1; mc[0]<=mofst[0]+thbk[0]; (mc[0])++)  // Note: added
